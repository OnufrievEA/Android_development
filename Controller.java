...
public class Controller implements Initializable {
    ...
    final int PORT = 8189;
    final String CLIENT_PATH = "./client/src/Client/";
    File history_file = null;
    String currentLogin;
    List<String> new_messages = new ArrayList<>();
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    //цикл работы
                    history_file = new File(CLIENT_PATH + currentLogin + ".txt");
                    readClientHistory();
...
//        System.out.println(clientList.getSelectionModel().getSelectedItem());
    ...
    private void readClientHistory() throws IOException {
        if (history_file.exists()) {
            try (BufferedReader br = new BufferedReader(new FileReader(history_file));) {
                String line;
                StringBuilder history_message = new StringBuilder();
                ...
                if (fileLines.size() <= 100) {
                    for (int i = 0; i < fileLines.size(); i++) {
                        history_message.append(fileLines.get(i) + "\n");
                    }
                } else {
                    for (int i = fileLines.size() - 100; i < fileLines.size(); i++) {
                        history_message.append(fileLines.get(i) + "\n");
                    }
                }
                textArea.appendText(history_message.toString());
            }
        }
    }

    private void writeChatHistory(List<String> message_list) throws IOException {
        try (BufferedWriter bf = new BufferedWriter(new FileWriter(history_file, true));) {
            for (int i = 0; i < message_list.size(); i++) {
                bf.write(message_list.get(i));
            }
        }
    }
}
